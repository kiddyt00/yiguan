      ...
      <div class="col-tl">
        <div ref="tlSection" class="tl-section">
          ...
          <div class="fishbone">
            ...
            <!-- 节点 -->
            <div v-for="(item, i) in timeline" :key="i" class="fb-node" :class="i % 2 === 0 ? 'fb-left' : 'fb-right'">
              <!-- 连接横线 -->
              <div class="fb-arm"></div>
              <!-- 节点圆点 -->
              <div class="fb-dot" :class="item.dotClass"></div>
              <!-- 内容卡片 -->
              <div class="fb-card">
                <div class="tl-header">
...
const timeline = [
  ...
  },
  {
    ...
    tasks: [
      { done: true, text: '小程序 API 地址 11 处更新' },
      { done: true, text: '安装 miniprogram-ci 命令行上传工具' },
      { done: true, text: '管理后台卦象弹窗修复' },
      { done: true, text: 'favicon.ico 404 修复' },
      { done: true, text: '小程序支付前端（miniapp-native + miniapp）' },
    ],
  },
  {
    date: '2026-07-17', tag: '完成', tagClass: 'tag-done', dotClass: 'dot-done',
    title: '支付系统：微信支付修复 + JSAPI',
    tasks: [
      { done: true, text: '支付二维码渲染修复（Vue nextTick）' },
      { done: true, text: 'JSAPI 小程序支付后端' },
  ...
  },
  {
    ...
    tasks: [
      ...
      { done: true, text: '回调地址 + URL 编码修复' },
      { done: true, text: '安全加固：CORS / 限流 / 错误清理' },
    ],
  },
  {
    ...
    tasks: [
      ...
      { done: true, text: '域名 zgjz.insightj.cn + SSL' },
    ],
  },
].map(item => ({
  ...item,
...
const todos = [
  ...
  },
  {
    ...
    items: [
      { done: false, text: '支付宝扫码支付接入', note: '等商户资料' },
      { done: false, text: '短信服务接入', note: '当前仅打印日志' },
    ],
  },
  {
    label: '🟢 优化项', color: '#667eea',
    items: [
      { done: false, text: '前端 SSE 流自动重连' },
      { done: false, text: '零停机部署方案' },
      { done: false, text: '订单后台管理页面' },
      { done: false, text: '数据库自动备份' },
    ],
  },
]
...
.fb-spine {
  ...
  width: 2px; background: linear-gradient(to bottom, #d4a853 0%, #e5ddd0 70%, #e5ddd0 100%);
