...
func (s *Store) ListAllHistory(limit, offset int, userID int64) ([]store.History, error) {
	...
	for rows.Next() {
		var h store.History
		if err := rows.Scan(&h.ID, &h.UserID, &h.Nickname, &h.Question, &h.PrimaryGua, &h.ChangingGua, &h.YaoPositions, &h.PrimaryYao, &h.ChangingYao, &h.TossData, &h.MasterYao, &h.Interpretation, &h.Lang, &h.CreatedAt); err != nil {
			return nil, err
		}
		list = append(list, h)
	}
...
func (s *Store) GetHistoryByID(id int64) (*store.History, error) {
	...
	err := s.db.QueryRow(
		...
		id,
	).Scan(&h.ID, &h.UserID, &h.Question, &h.PrimaryGua, &h.ChangingGua, &h.YaoPositions, &h.PrimaryYao, &h.ChangingYao, &h.TossData, &h.MasterYao, &h.Interpretation, &h.Lang, &h.CreatedAt)
