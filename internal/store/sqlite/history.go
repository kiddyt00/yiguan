...
func (s *Store) ListUsers(limit, offset int) ([]store.User, error) {
	rows, err := s.db.Query(
		"SELECT id, phone, nickname, avatar, address, created_at FROM users ORDER BY id DESC LIMIT ? OFFSET ?",
		limit, offset,
	)
	if err != nil {
		return nil, err
	...
	for rows.Next() {
		var u store.User
		if err := rows.Scan(&u.ID, &u.Phone, &u.Nickname, &u.Avatar, &u.Address, &u.CreatedAt); err != nil {
			return nil, err
		}
		list = append(list, u)
	}
